    ...
    try {
        ...
        try {
            ...
            if ($result -and (Test-Path $vhdxPath)) {
                ...
                # Re-read the file to get accurate size information
                $fileInfo = Get-Item $vhdxPath -ErrorAction SilentlyContinue
                if ($fileInfo) {
                    $sizeGB = [math]::Round($fileInfo.Length / 1GB, 2)
                    Write-Verbose "VHDX created: $($fileInfo.FullName) ($sizeGB GB)"
                } else {
                    Write-Verbose "VHDX created: $vhdxPath (size unavailable)"
                }
            } else {
